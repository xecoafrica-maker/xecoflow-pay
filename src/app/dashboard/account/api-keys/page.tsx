...
export default function DevelopersPage() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          setError('API credentials endpoint not found.');
          setLoading(false);
          return;
        }
      ...
      if (data.success && data.data) {
        ...
        setError('No API credentials found.');
      }
    } catch (error) {
  ...
  const displayApiKey = showApiKey ? apiKey : maskString(apiKey);
  const displaySecret = showSecret ? apiSecret : maskString(apiSecret);
          ...
          <div className="p-6">
            <div className="border border-gray-200 rounded-xl p-4 bg-white">
              <div className="flex items-center gap-3">
                <span className="text-sm font-mono bg-gray-50 px-3 py-2 rounded-lg border border-gray-200 flex-1 truncate">
                  {apiKey ? displayApiKey : 'No API key found'}
                </span>
                {apiKey && (
                  <>
                    <button
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
          ...
          <div className="p-6">
            <div className="border border-gray-200 rounded-xl p-4 bg-white">
              <div className="flex items-center gap-3">
                <span className="text-sm font-mono bg-gray-50 px-3 py-2 rounded-lg border border-gray-200 flex-1 truncate">
                  {apiSecret ? displaySecret : 'No API secret found'}
                </span>
                {apiSecret && (
                  <>
                    <button
