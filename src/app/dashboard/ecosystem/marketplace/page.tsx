...
export default function AppsMarketplacePage() {
  ...
  const handleOpenApp = useCallback((app: App) => {
    ...
    // Small delay only for visual feedback, not for data loading
    setTimeout(() => router.push(app.href), 400);
  }, [router]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-950">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 sm:pb-0" role="tablist" aria-label="Filter by category">
            {categories.map(cat => (
              <button
                key={cat}
        ...
        {featuredApps.length > 0 && selectedCategory === 'All' && !searchTerm && (
          <section className="mb-10" aria-labelledby="featured-heading">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 id="featured-heading" className="text-xl font-semibold text-gray-800 dark:text-gray-200">Featured Apps</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {featuredApps.map(app => (
                <div
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && handleOpenApp(app)}
                  tabIndex={app.status !== 'coming_soon' ? 0 : -1}
                  role="button"
                  aria-label={`Open ${app.name}`}
                  className="group relative bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-md hover:shadow-xl transition-all cursor-pointer border-2 border-emerald-500/20 hover:border-emerald-500/50 focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
                >
                  {app.badge && (
        ...
        <section aria-labelledby="all-apps-heading">
          <div className="flex items-center justify-between mb-4">
            <h2 id="all-apps-heading" className="text-xl font-semibold text-gray-800 dark:text-gray-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filteredApps.map(app => (
                <div
                  ...
                  role="button"
                  aria-label={`Open ${app.name}`}
                  className="group bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm hover:shadow-md transition-all cursor-pointer border border-gray-200 dark:border-slate-700 hover:border-emerald-500/30 focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
                >
      ...
      {loadingApp && (
        <div 
          className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-white/95 backdrop-blur-sm transition-all duration-500"
          role="status"
