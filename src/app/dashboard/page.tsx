...
export default function DashboardOverview() {
    ...
    try {
      ...
      if (data) {
        ...
        const mappedSteps = stepMappings.map((step) => ({
          ...step,
          icon: step.label.includes('Business Profile')
            ? Building
            : step.label.includes('Owners')
            ? Users
            : step.label.includes('Tax')
            ? FileText
            : step.label.includes('Settlement')
            ? Landmark
            : CheckCircle,
      ...
      // Onboarding status is non-blocking; ignore transient failures.
    }
  };
  ...
  const fetchDashboardData = async (merchantIdParam?: string) => {
    try {
      ...
      const transRes = await fetch(
        `/api/transactions/inflow?${params.toString()}`,
        { credentials: 'include' }
      );
      ...
      if (merchantIdParam) {
        ...
        const balanceRes = await fetch(
          `/api/ledger/accounts/${accountNumber}/balance`,
          { credentials: 'include' }
        );
      ...
      // Data fetch failures render an empty dashboard; the layout
      // handles session expiry separately.
    } finally {
  ...
  useEffect(() => {
    ...
    if (statusFilter !== 'All') {
      filtered = filtered.filter((t) => {
        const combined =
          `${t.status || ''} ${t.payment_status || ''}`.toUpperCase();
  ...
  }, [loading, merchantId, merchantName, log, ActivityActions]);
  ...
  const chartData = [...filteredTransactions]
    ...
    .slice(0, 7)
    .map((t) => ({
      day: new Date(t.created_at).toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric',
      }),
  ...
  const getTodayTransactions = () => {
    ...
    return todayTxs
      ...
      .map((t) => ({
        ...
        customer:
          t.phone_number ||
          (t.channel === 'C2B' ? 'M-PESA Paybill' : 'Unknown'),
        amount: parseFloat(t.amount || '0'),
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            ...
            <div className="space-y-2">
              {[1, 2, 3, 4, 5].map((i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 px-3 py-2.5 bg-gray-50 rounded-lg animate-pulse"
                >
    ...
    <div className="max-w-[1400px] mx-auto space-y-4">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-[13px] font-medium text-gray-800">
                Setup incomplete · {totalSteps - completedSteps} steps remaining
              </p>
            </div>
          </div>
      ...
      {showOnboarding && !isFullyOnboarded && onboardingSteps.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm relative">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pt-3 border-t border-gray-100">
            <Link
              href={
                onboardingSteps.find((s) => !s.completed)?.href || '/dashboard'
              }
        ...
        {statsData.map((stat) => (
          ...
          >
            ...
            <div className="flex items-center gap-1 text-[11px]">
              ...
              <span
                className={stat.trendUp ? 'text-emerald-600' : 'text-red-600'}
              >
                {stat.trend}
              </span>
            </div>
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-gray-500 mt-0.5">
                {filteredTransactions.filter(isCompleted).length > 0
                  ? `${
                      filteredTransactions.filter(isCompleted).length
            ...
            {chartData.length > 0 ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-gray-400 gap-2">
                ...
                <span className="text-[13px]">
                  No completed transactions to chart
                </span>
              </div>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-medium text-gray-900 group-hover:text-blue-700 transition-colors">
                  Create Bill Link
                </p>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-medium text-gray-900 group-hover:text-emerald-700 transition-colors">
                  Withdraw Funds
                </p>
                <p className="text-[11px] text-gray-400">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-medium text-gray-900 group-hover:text-purple-700 transition-colors">
                  API Integration
                </p>
                <p className="text-[11px] text-gray-400">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-medium text-gray-900 group-hover:text-amber-700 transition-colors">
                  Generate Statement
                </p>
                <p className="text-[11px] text-gray-400">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-medium text-gray-900 group-hover:text-rose-700 transition-colors">
                  Buy Airtime
                </p>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between">
          ...
          <Link
            href="/dashboard/inflow"
            className="text-[12px] text-emerald-500 font-medium hover:text-emerald-600"
          >
            View All →
          </Link>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 text-left bg-gray-50">
                <th className="px-5 py-2.5 text-[11px] font-medium text-gray-400 uppercase tracking-wider">
                  Transaction
                </th>
                <th className="px-5 py-2.5 text-[11px] font-medium text-gray-400 uppercase tracking-wider">
                  Customer
            ...
            <tbody>
              {recentTransactions.length > 0 ? (
                recentTransactions.map((tx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-2.5">
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium ${statusInfo.color}`}
                        >
                          {statusInfo.icon}
                          {statusInfo.label}
                        </span>
                ...
                <tr>
                  <td
                    colSpan={7}
                    className="px-5 py-6 text-center text-gray-400 text-[13px]"
                  >
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-mono text-[13px] mt-0.5">
                    {selectedTransaction.checkout_id || '—'}
                  </p>
                </div>
                <div>
                  <p className="text-[11px] text-gray-400 uppercase tracking-wider">
                  ...
                  <p className="font-bold text-[18px] text-gray-900 mt-0.5">
                    KES{' '}
                    {parseFloat(
                  ...
                  <div className="mt-1">
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium ${
                        getStatusDisplay(deriveStatus(selectedTransaction))
                          .color
                      }`}
                    >
                ...
                <div>
                  ...
                  <p className="text-[13px] mt-0.5">
                    {selectedTransaction.user_id || merchantId}
                  </p>
                </div>
                <div>
                  <p className="text-[11px] text-gray-400 uppercase tracking-wider">
                  ...
                  <p className="text-[13px] mt-0.5">
                    {selectedTransaction.source || 'M-PESA'}
                  </p>
                </div>
                <div>
                  <p className="text-[11px] text-gray-400 uppercase tracking-wider">
                  ...
                  <p className="text-[13px] mt-0.5">
                    {selectedTransaction.channel === 'C2B'
                      ? 'M-PESA Paybill'
                      : 'M-PESA STK Push'}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-[13px] mt-0.5">
                    {selectedTransaction.request_type}
                  </p>
                </div>
